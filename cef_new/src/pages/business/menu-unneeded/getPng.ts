import { ItemType, ItemId } from '#/shared/data/itemsInfo';
...
const Bool = (text: string): boolean => {
  return text.toLowerCase() === "true";
};

export const getPngToItemId = (data: ItemData): { name: string; png: string } => {
  let title = data.Name;
  let png = data.Png;
  ...
  switch (data.Type) {
    case 0:
      ...
      png = getPng({ ItemId: data.ItemId, Data: data.Data }, itemInfo);
...
export const getPng = (localItem: LocalItem, iconInfo: ItemInfo): string => {
  try {
    ...
    if (e instanceof Error) {
      console.log("Error in getPng: " + e.message);
    } else {
      console.log("Unknown error occurred in getPng");
    }
    return "";
  }
};